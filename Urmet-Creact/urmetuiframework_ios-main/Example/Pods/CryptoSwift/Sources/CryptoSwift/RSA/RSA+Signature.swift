...
extension RSA: Signature {
    public func sign(_ bytes: ArraySlice<UInt8>) throws -> [UInt8] {
        try sign(Array(bytes), variant: .message_pkcs1v15_SHA256)
    }
    ...
    public func sign(_ bytes: [UInt8], variant: SignatureVariant) throws -> [UInt8] {
        ...
        // Hash & Encode Message
        let hashedAndEncoded = try RSA.hashedAndEncoded(bytes, variant: variant, keySizeInBytes: keySizeBytes)

        /// Calculate the Signature
        let signedData = BigUInteger(Data(hashedAndEncoded)).power(d, modulus: n).serialize().bytes

        return variant.formatSignedBytes(signedData, blockSize: keySizeBytes)
    }

    public func verify(signature: ArraySlice<UInt8>, for expectedData: ArraySlice<UInt8>) throws -> Bool {
        try verify(signature: Array(signature), for: Array(expectedData), variant: .message_pkcs1v15_SHA256)
    }
    ...
    public func verify(signature: [UInt8], for bytes: [UInt8], variant: SignatureVariant) throws -> Bool {
        ...
        guard signature.count == keySizeBytes else { throw Error.invalidSignatureLength }

        /// Prepare the expected message for signature comparison
        var expectedData = try RSA.hashedAndEncoded(bytes, variant: variant, keySizeInBytes: keySizeBytes)
        if expectedData.count == keySizeBytes && expectedData.prefix(1) == [0x00] { expectedData = Array(expectedData.dropFirst()) }
        ...
        internal func encode(_ bytes: [UInt8]) -> [UInt8] {
            ...
            default:
                let asn: ASN1.Node = .sequence(nodes: [
                    .sequence(nodes: [
                        .objectIdentifier(data: Data(identifier)),
                        .null,
                    ]),
