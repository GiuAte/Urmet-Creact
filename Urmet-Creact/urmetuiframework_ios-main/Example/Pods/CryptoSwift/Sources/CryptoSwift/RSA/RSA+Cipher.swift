...
extension RSA: Cipher {
    ...
    public func encrypt(_ bytes: ArraySlice<UInt8>) throws -> [UInt8] {
        return try encrypt([UInt8](bytes), variant: .pksc1v15)
    }

    @inlinable
    public func encrypt(_ bytes: [UInt8], variant: RSAEncryptionVariant) throws -> [UInt8] {
        // Prepare the data for the specified variant
        let preparedData = try variant.prepare(bytes, blockSize: keySizeBytes)

        // Encrypt the prepared data
        return try variant.formatEncryptedBytes(encryptPreparedBytes(preparedData), blockSize: keySizeBytes)
    }
    ...
    public func decrypt(_ bytes: ArraySlice<UInt8>) throws -> [UInt8] {
        return try decrypt([UInt8](bytes), variant: .pksc1v15)
    }

    @inlinable
    public func decrypt(_ bytes: [UInt8], variant: RSAEncryptionVariant) throws -> [UInt8] {
        // Decrypt the data
        let decrypted = try decryptPreparedBytes(bytes)

        // Remove padding / unstructure data and return the raw plaintext
        return variant.removePadding(decrypted, blockSize: keySizeBytes)
    }
