...
public final class Scrypt {
    ...
    public func calculate() throws -> [UInt8] {
        ...
        let barray = try PKCS5.PBKDF2(password: Array(password), salt: Array(salt), iterations: 1, keyLength: p * 128 * r, variant: .sha2(.sha256)).calculate()
        ...
        return try PKCS5.PBKDF2(password: Array(password), salt: block, iterations: 1, keyLength: dkLen, variant: .sha2(.sha256)).calculate()
    }
}
