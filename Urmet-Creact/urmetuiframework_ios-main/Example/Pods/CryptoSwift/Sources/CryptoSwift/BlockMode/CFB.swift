...
public struct CFB: BlockMode {
    ...
    public init(iv: [UInt8], segmentSize: SegmentSize = .cfb128) {
        ...
        customBlockSize = segmentSize.rawValue
    }
    ...
    public func worker(blockSize: Int, cipherOperation: @escaping CipherOperationOnBlock, encryptionOperation _: @escaping CipherOperationOnBlock) throws -> CipherModeWorker {
        if !(iv.count == blockSize || (segmentSize == .cfb8 && iv.count == AES.blockSize)) {
            throw Error.invalidInitializationVector
        }

        return CFBModeWorker(blockSize: blockSize, iv: iv.slice, segmentSize: segmentSize, cipherOperation: cipherOperation)
    }
}
...
struct CFBModeWorker: BlockModeWorker {
    ...
    mutating func encrypt(block plaintext: ArraySlice<UInt8>) -> [UInt8] {
        ...
        case .cfb8:
            ...
            prev = Array((prev ?? iv).dropFirst()) + [result[0]]
            return result
        }
    }
    ...
    mutating func decrypt(block ciphertext: ArraySlice<UInt8>) -> [UInt8] {
        ...
        case .cfb8:
            ...
            prev = Array((prev ?? iv).dropFirst()) + [Array(ciphertext)[0]]
