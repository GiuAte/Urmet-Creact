...
public final class Blowfish {
    ...
    private func setupBlockModeWorkers() throws {
        ...
        encryptWorker = try blockMode.worker(blockSize: Blowfish.blockSize, cipherOperation: encryptBlock, encryptionOperation: encryptBlock)

        if blockMode.options.contains(.useEncryptToDecrypt) {
            decryptWorker = try blockMode.worker(blockSize: Blowfish.blockSize, cipherOperation: encryptBlock, encryptionOperation: encryptBlock)
        } else {
            decryptWorker = try blockMode.worker(blockSize: Blowfish.blockSize, cipherOperation: decryptBlock, encryptionOperation: encryptBlock)
        }
    }
